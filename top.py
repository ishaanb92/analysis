...
for run in range(2,6):
    ...
    test_means_dict,train_means_dict,_,_ = analyze_embeddings(run=str(run),draw=True,log_file=log_file)
    emb_test_dist_means.append(test_means_dict)
    emb_train_dist_means.append(train_means_dict)
    log_file.write('\n')
    log_file.write('*** Metric Analysis ***\n')
    run_mean_dict = analyze_metric(run=str(run),draw=True,log_file=log_file)
