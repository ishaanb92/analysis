...
from analyze_metric import analyze_metric
from stats import *
...
def calculate_mean_stats(last_run,draw,log_file,dataset='mnist'):
    """
    ...
    for run_id in range(1,last_run+1):
        log_file.write('### Analysis for Run {} ### \n\n'.format(run_id))
        log_file.write('*** Embeddings Analysis ***\n')
        test_means_dict,train_means_dict,_,_ = analyze_embeddings(run=str(run_id),draw=draw,log_file=log_file,dataset=dataset)
        emb_test_dist_means.append(test_means_dict)
        emb_train_dist_means.append(train_means_dict)
        log_file.write('\n')
        log_file.write('*** Metric Analysis ***\n')
        run_mean_dict = analyze_metric(run=str(run_id),draw=draw,log_file=log_file,dataset=dataset)
    ...
    for model in models:
        ...
        avg_gap[model] = abs(emb_test_mean-emb_train_mean)
        std_gap[model] = np.asarray(gap_means).std()

        # For a single run, the "inter-run" stddev will be zero !
...
def accumulate_scores(last_run,draw,log_file,dataset='mnist'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset',type=str,help='Dataset to analyze: celeba or mnist',required=True)
    ...
    log_dir = os.path.join(os.getcwd(),'logs',args.dataset.lower())
    if os.path.exists(log_dir) is False:
        os.makedirs(os.path.join(log_dir))

    log_file = open(os.path.join(log_dir,'log_file.txt'),'w')
    ...
    calculate_mean_stats(last_run=last_run,draw=draw,log_file=log_file,dataset=args.dataset)
    accumulate_scores(last_run=last_run,draw=draw,log_file=log_file,dataset=args.dataset)
